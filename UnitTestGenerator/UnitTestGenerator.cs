...
public static class UnitTestGenerator
{
    public static void Main()
    {
        ...
        List<TestOffset> offsets = [ 
            TestOffset.PositiveQuadrant, 
            TestOffset.CenteredAtOrigin 
        ];
        ...
        {
            foreach (TestOffset offset in offsets)
            {
                foreach ((string testName, TestPurpose testPurpose) in variants)
                {
                    string fullTestName = testName + "_" + OffsetToName() + "_" + BorderToName();

                    Console.WriteLine("Outputting test " + fullTestName + "...");

                    string output = testLayoutParser.GenerateCode(fullTestName, testPurpose, border, offset);
                    ...
                    [Pure] string OffsetToName() => offset == TestOffset.PositiveQuadrant ? "PositiveQuadrant" : "CenteredAtOrigin";
                }
            }
        }
